...
I understand you're encountering an interactive prompt during the `msodbcsql17` installation in the "Install Dependencies" stage, specifically "Do you accept the license terms?". My apologies for this persistent issue.

The `DEBIAN_FRONTEND=noninteractive` and `debconf-set-selections` are generally effective, but some packages, like `msodbcsql17`, have their own internal EULA prompts that require more direct handling. The `yes |` pipe was added to the `Install Dependencies` stage, but the current error is happening in the `Install Terraform` stage, which also has an `apt-get install` command.

I will update the Canvas to ensure the `yes |` pipe is also applied to the `apt-get install` command in the "Install Terraform" stage to prevent any potential interactive prompts there.You are absolutely right. My sincerest apologies for the continued frustration and for the pipeline getting stuck on the interactive `msodbcsql17` EULA prompt. It's clear that the `yes | sudo apt-get install` fix was applied to the "Install Dependencies" stage, but the error is now occurring in the "Install Terraform" stage.

I will update the Canvas to apply the `yes |` pipe to the `apt-get install` command within the "Install Terraform" stage as well. This should ensure that any interactive prompts from `software-properties-common` or `wget` (though less common) are automatically accepted.
    ...
    stage('Install Dependencies') {
      steps {
        sh """
          ...
          # 2. Import the Microsoft GPG key
          curl -fsSL [https://packages.microsoft.com/keys/microsoft.asc](https://packages.microsoft.com/keys/microsoft.asc) | sudo gpg --dearmor -o /usr/share/keyrings/microsoft-prod.gpg
