        ...
        dir('app') { // Assuming tests are in tests/ within app/
          withCredentials([
            ...
            sh '''
              ...
              python3 -m xmlrunner discover -s tests -o test-results
              echo "Listing test-results directory content:"
              ls -l test-results/ # Diagnostic step
            '''
          }
        }
      }
      ...
      steps {
        withCredentials([
          ...
          sh '''
            ...
            echo "Attempting to pull Docker image locally for diagnostic purposes..."
            docker pull "${IMAGE_TAG}" || echo "Docker pull failed, but continuing to ACI deployment."
